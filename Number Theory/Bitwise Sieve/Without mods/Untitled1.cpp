...
{
    int i, j;
    for(i = 4; i <= n; i += 2) chk[i/32] = Set(chk[i/32], i%32);

    for(i = 3; ll(i)*ll(i) <= ll(n); i++){
        if(Check(chk[i/32], i%32)) continue;
        for(j = i*i; j <= n; j += i){
            chk[j/32] = Set(chk[j/32], j%32);
        }
    }
    for(int i = 2; i <= n; i++) if(!Check(chk[i/32], i%32)) prime.push_back(i);
    ///O(n*log2(log2(n))
}
